...
if __name__ == "__main__":
    ...
    if args.mode == "LSTM":
        model = nm.LSTMSequence(args.embedDim,args.gpuNr)
        trainer = nt.LSTMTrainer(args.batchSize,30,args.learningRate)
        best = trainer.train(model)
        trainer.test(best)
        torch.save(best,"LSTM_Model{}.pt".format(args.saveAs))
    elif args.mode == "Transformer":
        model = nm.TransformerModel(6,6,args.feedforwardDim,0.2,args.gpuNr)
        trainer = nt.IndividualSequenceTrainer(args.batchSize,30,args.learningRate)
        best = trainer.train(model)
        trainer.test(best)
        torch.save(best,"Transformer_Model{}.pt".format(args.saveAs))
    elif args.mode == "Ensemble":
        model = nm.Ensemble(args.embedDim,6,6,args.feedforwardDim,0.2,args.gpuNr)
        trainer = nt.EnsembleTrainer(args.batchSize,30,args.learningRate)
        best = trainer.train(model)
        trainer.test(best)
        torch.save(best,"Ensemble_Model{}.pt".format(args.saveAs))
